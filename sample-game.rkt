...
(define-struct (magic-8-bomb magic-8-ball)
  ;; sayings: (listof string)
  ;; The collection of strings that it picks from when generating an answer. 
  (defused?)
  
  #:methods
  ;; defuse: magic-8-bomb -> void
  ;; Mark the bomb defused
  (define (defuse bomb)
    (set-magic-8-bomb-defused?! bomb #t))
  
  ;; shake: magic-8-ball -> void
  ;; Randomly print a saying.
  (define (shake bomb)
    (if (magic-8-bomb-defused? bomb)
        (display-line "Nothing happens")
        (error "Boom!  You're dead!")))

  (define (examine bomb)
    (display-line "It's ticking suspiciously...")))

;; new-magic-8-bomb: container -> magic-8-ball
;; Makes a magic 8 ball with the specified set of sayings
(define (new-magic-8-bomb location)
  (local [(define ball
            (make-magic-8-bomb '("magic" "8")
                               '()
                               location
                               '()
                               #f))]
...
(define-user-command (defuse bomb) "Defuses the bomb")
